...
public class PersistenceUtil implements Serializable {
	protected static EntityManagerFactory emf = Persistence
			.createEntityManagerFactory("dt340a");
	...
	/**
	 * Persists a List of objects in the database. Only requires one
	 * connection/transaction.
	 * 
	 * @param entities
	 *            List of objects to be stored in the database
	 */
	...
	public static void dropSecondaryTables() {
		...
		em.createNativeQuery(
				"DROP TABLE IF EXISTS EventCause, FailureClass, MCC_MNC, UEType")
				.executeUpdate();
		em.getTransaction().commit();
		em.close();
	}
	...
	/**
	 * Returns the total number of ErrorEvent entries in the ErrorEvent table in
	 * the database
	 * 
	...
	public static long numberOfErrorEvents() {
		EntityManager em = emf.createEntityManager();
		em.getTransaction().begin();
		long count = ((BigInteger) em.createNativeQuery(
				"SELECT COUNT(*) FROM ErrorEvent").getSingleResult())
				.longValue();
		em.getTransaction().commit();
		em.close();
	...
	/**
	 * Returns the total number of InvalidErrorEvent entries in the
	 * InvalidErrorEvent table in the database
	 * 
	...
	public static long numberOfInvalidErrorEvents() {
		...
		em.getTransaction().begin();
		long count = ((BigInteger) em.createNativeQuery(
				"SELECT COUNT(*) FROM InvalidErrorEvent").getSingleResult())
				.longValue();
		em.getTransaction().commit();
		em.close();
	...
	public static EventCause findEventCauseByEventIdAndCauseCode(int eventId,
			int causeCode) {
		EntityManager em = emf.createEntityManager();
		List<EventCause> eventCauses = (List<EventCause>) em
				.createNamedQuery("EventCause.findByEventIdAndCauseCode")
	...
	public static MCC_MNC findMCCMNCByMCCAndMNC(int mcc, int mnc) {
		EntityManager em = emf.createEntityManager();
		List<MCC_MNC> mcc_mncs = (List<MCC_MNC>) em
				.createNamedQuery("MCC_MNC.findByMCCANDMNC")
				.setParameter("id", new MCCMNCComp(mcc, mnc)).getResultList();
		em.close();
	...
	public static List<EventCause> findEventCauseByIMSI(long imsi) {
		EntityManager em = emf.createEntityManager();
		List<EventCause> eventCauses = (List<EventCause>) em
				.createNamedQuery("ErrorEvent.EventCauseByIMSI")
	...
	public static UserType findUserTypeByUserTypeId(int userTypeId) {
